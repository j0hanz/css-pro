...
Every plan written by `motion-advisor`'s audit mode follows this structure. The executor may be a less capable model with zero context and zero taste — the plan must contain everything, exactly. No references to "the audit above" or "the easing we discussed."
...
configs, media queries. Pulled from `motion-craft` (the craft bar), never
approximated from memory. Never "use a nicer easing":
...
- Pull every value from `motion-craft` (the craft bar) — never approximate from memory. For gesture physics (velocity handoff, momentum projection, rubber-banding), pull from `motion-foundations`.
