...
Layout layer: grid/flex patterns adapt to content and container without media queries, one-line property upgrades retire old hacks, few selector tools worth stating. Custom properties: see [`PROPERTIES.md`](PROPERTIES.md). Process: see [`SKILL.md`](SKILL.md).
...
- **Intrinsic grid** — columns that wrap on their own:
  ...
  Inner `min(100%, ...)` load-bearing: without it, containers narrower than `--grid-min` overflow.
...
- **Equal heights** — free: flex children stretch by default; grid `grid-auto-flow: column` give equal height _and_ width. No `height: 100%` hacks on parent — only on nested card innards.
...
Only tools changing how system composes — rest standard.
...
- **`@layer`** — declare order once, `@layer reset, theme, components, utilities;` — first listed loses. Trap: _unlayered_ styles beat all layers; `@supports` cannot detect at-rules. Put `@property` registrations in layer to control override precedence.
