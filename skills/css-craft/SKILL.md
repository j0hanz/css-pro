...
**Invalid `var()` cannot fall back to earlier declaration.** Those discarded at parse time, property falls to inherited or initial value instead — `color: blue; color: var(--broken)` gives inherited colour, not blue. Give every `var()` a fallback of the right type — it rescues the _missing_ token only. `var(--foo, red, blue)` is one fallback of `red, blue` — everything after first comma is the fallback. Gotchas in [`PROPERTIES.md`](PROPERTIES.md).
